...
hex_colors = [hex_colors[i] for i in select_list]
rgb_colors = [ordered_colors[i] for i in select_list]

#count_quan = list(counts.values())
#count_quan = [count_quan[i] for i in select_list]
count_quan = [25, 25, 25, 25]
...
axs[1].pie(count_quan, labels = hex_colors, colors = hex_colors)
axs[2].set_title("Crawler")
axs[2].pie(count_quan, labels = hex_colors_real, colors = hex_colors_real)
plt.show()
